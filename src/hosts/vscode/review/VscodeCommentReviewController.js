 ...
 */
export class VscodeCommentReviewController extends CommentReviewController {
	commentController
	threads = new Map()
	/** Maps thread to its absolute file path (needed because virtual URIs don't contain the full path) */
	threadFilePaths = new Map()
	onReplyCallback
	disposables = []
	...
	constructor() {
		...
		this.disposables.push(
			vscode.commands.registerCommand("cline.reviewComment.reply", async (reply) => {
				await this.handleReply(reply)
			}),
		)
		// Register add to chat command - sends the conversation to Cline's main chat
		this.disposables.push(
			vscode.commands.registerCommand("cline.reviewComment.addToChat", async (thread) => {
	...
	async ensureCommentsViewDisabled() {
		...
		const currentValue = config.get("openView")
	 ...
	 */
	startStreamingComment(filePath, startLine, endLine, relativePath, fileContent, revealComment = false) {
		// Use virtual diff URI if relativePath and fileContent are provided
		let uri
	 ...
	 */
	async revealCommentInDocument(thread) {
		try {
	...
	async handleReply(reply) {
		...
		if (this.onReplyCallback) {
			...
			const updateStreamingComment = (content) => {
...
Please continue helping the user with their question about this code.`
	...
	getThreadKey(filePath, startLine, endLine) {
 ...
 */
export function getVscodeCommentReviewController() {
	if (!instance) {
 ...
 */
export function disposeVscodeCommentReviewController() {
	if (instance) {
		instance.dispose()
		instance = undefined
	}
}
//# sourceMappingURL=VscodeCommentReviewController.js.map
