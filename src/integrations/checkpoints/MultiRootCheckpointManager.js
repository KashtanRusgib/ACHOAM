...
export class MultiRootCheckpointManager {
    ...
    initialized = false;
    initPromise;
    constructor(workspaceManager, taskId, enableCheckpoints, messageStateHandler) {
    ...
    async doInitialize() {
        ...
        telemetryService.captureMultiRootCheckpoint(this.taskId, "initialized", roots.length, successCount, failureCount, performance.now() - startTime);
    }
    /**
    ...
    async saveCheckpoint() {
        ...
        Promise.all(commitPromises)
            ...
            telemetryService.captureMultiRootCheckpoint(this.taskId, "committed", results.length, successful, failed, performance.now() - startTime);
        })
            .catch((error) => {
    ...
    async commit() {
        ...
        if (!primaryRoot) {
            ...
            const commitPromises = Array.from(this.trackers.values()).map((tracker) => tracker.commit().catch((error) => {
...
//# sourceMappingURL=MultiRootCheckpointManager.js.map