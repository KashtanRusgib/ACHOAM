...
export class TaskCheckpointManager {
    ...
    taskState;
    state;
    constructor(task, config, services, callbacks, initialState) {
     ...
     */
    async saveCheckpoint(isAttemptCompletionMessage = false, completionMessageTs) {
        try {
            ...
            else if (!this.state.checkpointTracker &&
            ...
            if (!this.state.checkpointTracker) {
                console.error(`[TaskCheckpointManager] Failed to save checkpoint for task ${this.task.taskId}: Checkpoint tracker not available`);
                return;
            }
            ...
            if (!isAttemptCompletionMessage) {
                ...
                if (messageTs) {
                    ...
                    if (targetMessage) {
                        this.state.checkpointTracker
                            ...
                            .catch((error) => {
                            console.error(`[TaskCheckpointManager] Failed to create checkpoint commit for task ${this.task.taskId}:`, error);
                        });
                    }
                }
            }
                ...
                if (this.state.checkpointTracker) {
                    ...
                    console.error(`[TaskCheckpointManager] Checkpoint tracker does not exist and could not be initialized for attempt completion for task ${this.task.taskId}`);
                }
            }
        }
        catch (error) {
    ...
    async restoreCheckpoint(messageTs, restoreType, offset) {
        try {
                ...
                case "workspace":
                    ...
                    if (!this.state.checkpointTracker && !this.state.checkpointManagerErrorMessage) {
                        try {
                            ...
                            this.state.checkpointTracker = await CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath);
                            this.services.messageStateHandler.setCheckpointTracker(this.state.checkpointTracker);
                        }
                        catch (error) {
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            console.error(`[TaskCheckpointManager] Failed to initialize checkpoint tracker for task ${this.task.taskId}:`, errorMessage);
                    ...
                    if (message.lastCheckpointHash && this.state.checkpointTracker) {
                        ...
                        try {
                            ...
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            console.error(`[TaskCheckpointManager] Failed to restore offset checkpoint for task ${this.task.taskId}:`, errorMessage);
                            HostProvider.window.showMessage({
                                type: ShowMessageType.ERROR,
                        ...
                        console.warn(`[TaskCheckpointManager] Message ${messageTs} has no checkpoint hash, falling back to previous checkpoint for task ${this.task.taskId}`);
                        try {
                            await this.state.checkpointTracker.resetHead(lastMessageWithHash.lastCheckpointHash);
                        }
                        catch (error) {
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            console.error(`[TaskCheckpointManager] Failed to restore fallback checkpoint for task ${this.task.taskId}:`, errorMessage);
                            HostProvider.window.showMessage({
                                type: ShowMessageType.ERROR,
                                message: "Failed to restore checkpoint: " + errorMessage,
                            });
            ...
            const checkpointManagerStateUpdate = {};
    ...
    async presentMultifileDiff(messageTs, seeNewChangesSinceLastTaskCompletion) {
        ...
        try {
            ...
            if (!hash) {
                console.error(`[TaskCheckpointManager] No checkpoint hash found for message ${messageTs} in task ${this.task.taskId}`);
                relinquishButton();
                return;
            }
            ...
            if (seeNewChangesSinceLastTaskCompletion) {
                ...
                const lastTaskCompletedMessageCheckpointHash = findLast(this.services.messageStateHandler.getClineMessages().slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash;
    ...
    async commit() {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : "Unknown error";
            console.error(`[TaskCheckpointManager] Failed to create checkpoint commit for task ${this.task.taskId}:`, errorMessage);
            return undefined;
        }
    }
    /**
    ...
    async doesLatestTaskCompletionHaveNewChanges() {
        try {
            ...
            const hash = message.lastCheckpointHash;
            if (!hash) {
                console.error(`[TaskCheckpointManager] No checkpoint hash found for completion message in task ${this.task.taskId}`);
                return false;
            }
            ...
            // Get last task completed
            const lastTaskCompletedMessage = findLast(this.services.messageStateHandler.getClineMessages().slice(0, messageIndex), (m) => m.say === "completion_result");
            // Get last task completed
    ...
    async handleSuccessfulRestore(restoreType, message, messageIndex, messageTs) {
        switch (restoreType) {
            case "task":
            case "taskAndWorkspace":
                ...
                if (restoreType === "task") {
                    const filesEditedAfterMessage = await this.services.fileContextTracker.detectFilesEditedAfterMessage(messageTs, deletedMessages);
                ...
                await this.callbacks.say("deleted_api_reqs", JSON.stringify({
     ...
     */
    async checkpointTrackerCheckAndInit() {
        ...
        try {
            checkpointsWarningTimer = setTimeout(async () => {
                if (!checkpointsWarningShown) {
                    ...
                    await this.setcheckpointManagerErrorMessage("Checkpoints are taking longer than expected to initialize. Working in a large repository? Consider re-opening Cline in a project that uses git, or disabling checkpoints.");
                }
            }, 7_000);
            // Timeout - If checkpoints take too long to initialize, warn user and disable checkpoints for the task
            const workspacePath = await this.getWorkspacePath();
            const tracker = await pTimeout(CheckpointTracker.create(this.task.taskId, this.config.enableCheckpoints, workspacePath), {
                milliseconds: 15_000,
            ...
            if (errorMessage.includes("Checkpoints taking too long to initialize")) {
                await this.setcheckpointManagerErrorMessage("Checkpoints initialization timed out. Consider re-opening Cline in a project that uses git, or disabling checkpoints.");
            }
            else {
                await this.setcheckpointManagerErrorMessage(errorMessage);
            }
     ...
     */
    updateConversationHistoryDeletedRange(range) {
    ...
    async getWorkspacePath() {
        ...
        if (this.services.workspaceManager) {
            try {
                ...
                console.warn(`[TaskCheckpointManager] Failed to get workspace path from WorkspaceRootManager for task ${this.task.taskId}:`, error);
            }
        }
 ...
 */
export function createTaskCheckpointManager(task, config, services, callbacks, initialState) {
