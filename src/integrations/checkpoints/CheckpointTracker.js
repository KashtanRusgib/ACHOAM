...
class CheckpointTracker {
     ...
     */
    async sendCheckpointSubscriptionEvent(operation, isActive, commitHash) {
        try {
     ...
     */
    constructor(taskId, cwd, cwdHash) {
        this.taskId = taskId;
        this.cwd = cwd;
     ...
     */
    static async create(taskId, enableCheckpointsSetting, workspacePaths) {
        try {
    ...
    async commit() {
        ...
        try {
            ...
            const startTime = performance.now();
            const lockResult = await tryAcquireCheckpointLockWithRetry(this.cwdHash, this.taskId);
            // Locking failed due to conflicting lock
            if (!lockResult.acquired && !lockResult.skipped) {
                throw new Error("Failed to acquire checkpoint folder lock - another Cline instance may be performing checkpoint operations");
            }
