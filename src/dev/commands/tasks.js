 ...
 */
export function registerTaskCommands(controller) {
    return [
        vscode.commands.registerCommand("cline.dev.expireMcpOAuthTokens", async () => {
            try {
                ...
                const action = await vscode.window.showInformationMessage(`Expired ${expiredCount} MCP OAuth token(s). Reload window to test token refresh flow.`, "Reload Window", "Cancel");
