...
function setupHostProvider(extensionContext, extensionDir, dataDir) {
    const createWebview = () => {
    ...
    const getBinaryLocation = async (name) => path.join(process.cwd(), name);
    HostProvider.initialize(createWebview, createDiffView, createCommentReview, new ExternalHostBridgeClientManager(), log, getCallbackUrl, getBinaryLocation, extensionDir, dataDir);
}
/**
 ...
 */
async function requestHostBridgeShutdown() {
    try {
 ...
 */
async function shutdownGracefully(lockManager) {
    try {
