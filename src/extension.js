...
import { disposeVscodeCommentReviewController, getVscodeCommentReviewController, } from "./hosts/vscode/review/VscodeCommentReviewController";
...
export async function activate(context) {
    ...
    HookDiscoveryCache.getInstance().initialize(context, // Adapt VSCode ExtensionContext to generic interface
    (dir) => {
        try {
    ...
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(VscodeWebviewProvider.SIDEBAR_ID, webview, {
        webviewOptions: { retainContextWhenHidden: true },
    }));
    const { commands } = ExtensionRegistryInfo;
    context.subscriptions.push(vscode.commands.registerCommand(commands.PlusButton, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.McpButton, () => {
        sendMcpButtonClickedEvent();
    }));
    context.subscriptions.push(vscode.commands.registerCommand(commands.SettingsButton, () => {
        sendSettingsButtonClickedEvent();
    }));
    context.subscriptions.push(vscode.commands.registerCommand(commands.HistoryButton, async () => {
        // Send event to all subscribers using the gRPC streaming method
        await sendHistoryButtonClickedEvent();
    }));
    context.subscriptions.push(vscode.commands.registerCommand(commands.AccountButton, () => {
        // Send event to all subscribers using the gRPC streaming method
    ...
    */
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.TerminalOutput, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.AddToChat, async (range, diagnostics) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.FixWithCline, async (range, diagnostics) => {
        const context = await getContextForCommand(range, diagnostics);
        if (!context) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.ExplainCode, async (range) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.ImproveCode, async (range) => {
        const context = await getContextForCommand(range);
        if (!context) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.FocusChatInput, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.Walkthrough, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.ReconstructTaskHistory, async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.GenerateCommit, async (scm) => {
...
function setupHostProvider(context) {
    ...
    HostProvider.initialize(createWebview, createDiffView, createCommentReview, vscodeHostBridgeClient, outputChannel.appendLine, getCallbackUrl, getBinaryLocation, context.extensionUri.fsPath, context.globalStorageUri.fsPath);
}
async function getBinaryLocation(name) {
    ...
    const checkPath = async (pkgFolder) => {
        const fullPathResult = workspaceResolver.resolveWorkspacePath(vscode.env.appRoot, path.join(pkgFolder, name), "Services.ripgrep.getBinPath");
        const fullPath = typeof fullPathResult === "string" ? fullPathResult : fullPathResult.absolutePath;
        return (await fileExistsAtPath(fullPath)) ? fullPath : undefined;
    };
    const binPath = (await checkPath("node_modules/@vscode/ripgrep/bin/")) ||
