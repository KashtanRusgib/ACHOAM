...
export class BannerService {
    ...
    _baseUrl = ClineEnv.config().apiBaseUrl;
    _cachedBanners = [];
    _lastFetchTime = 0;
     ...
     */
    async fetchActiveBanners(forceRefresh = false) {
        try {
     ...
     */
    async evaluateBannerRules(banner) {
        try {
            ...
            if (rules.ide && rules.ide.length > 0) {
                ...
                if (currentIde && !rules.ide.includes(currentIde)) {
                    Logger.log(`BannerService: Banner ${banner.id} filtered out by IDE rule (requires: ${rules.ide.join(", ")}, current: ${currentIde})`);
                    return false;
                }
            }
            ...
            if (rules.auth && rules.auth.length > 0 && this._controller) {
                ...
                if (authProvider && !rules.auth.includes(authProvider)) {
                    Logger.log(`BannerService: Banner ${banner.id} filtered out by auth rule (requires: ${rules.auth.join(", ")}, current: ${authProvider})`);
                    return false;
                }
            }
            ...
            if (rules.providers && rules.providers.length > 0 && this._controller) {
                ...
                const hasAnyProvider = rules.providers.some((provider) => {
                    switch (provider) {
                        ...
                        case "cline":
                            return (apiConfiguration?.planModeApiProvider === "cline" ||
                ...
                if (!hasAnyProvider) {
                    Logger.log(`BannerService: Banner ${banner.id} filtered out - user doesn't have any of these providers configured: ${rules.providers.join(", ")}`);
                    return false;
                }
            }
            ...
            Logger.log(`BannerService: Error evaluating rules for banner ${banner.id}: ${error instanceof Error ? error.message : String(error)}`);
            return true;
        }
    }
    /**
     ...
     */
    isWithinActiveDateRange(banner) {
        const now = new Date();
    ...
    isUserTeamAdmin() {
        try {
            ...
            return organizations.some((org) => org.roles && (org.roles.includes("admin") || org.roles.includes("owner")));
        }
        catch (error) {
     ...
     */
    async getNonDismissedBanners(forceRefresh = false) {
