  ...
  "mcpServers": {
    ...
    async connectToServer(name, config, source) {
        ...
        try {
            ...
            const authProvider = config.type === "sse" || config.type === "streamableHttp"
    ...
    async clearOAuthForConnection(name) {
        const connection = this.connections.find((conn) => conn.server.name === name);
        if (connection) {
            try {
    ...
    async callTool(serverName, toolName, toolArguments, ulid) {
        ...
        try {
            ...
            this.telemetryService.captureMcpToolCall(ulid, serverName, toolName, "success", undefined, toolArguments ? Object.keys(toolArguments) : undefined);
            return {
                ...result,
            ...
            this.telemetryService.captureMcpToolCall(ulid, serverName, toolName, "error", error instanceof Error ? error.message : String(error), toolArguments ? Object.keys(toolArguments) : undefined);
            throw error;
        }
    }
    /**
    ...
    async addRemoteServer(serverName, serverUrl, transportType = "streamableHttp") {
        try {
            const settings = await this.readAndValidateMcpSettingsFile();
            if (!settings) {
     ...
     */
    async initiateOAuth(serverName) {
        const connection = this.connections.find((conn) => conn.server.name === serverName);
        if (!connection) {
            throw new Error(`No connection found for server: ${serverName}`);
        }
     ...
     */
    async completeOAuth(serverHash, code, state) {
