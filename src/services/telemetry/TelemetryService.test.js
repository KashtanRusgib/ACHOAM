...
describe("Telemetry system is abstracted and can easily switch between providers", () => {
    ...
    describe("No-Op Provider", () => {
        ...
        it("should handle unsupported provider types by returning No-Op provider", async () => {
            ...
            assert.ok(unsupportedProvider instanceof NoOpTelemetryProvider, "Unsupported provider should be an instance of NoOpTelemetryProvider");
            assert.strictEqual(unsupportedProvider.isEnabled(), false, "Unsupported provider should return NoOp provider");
            assert.deepStrictEqual(unsupportedProvider.getSettings(), {
                extensionEnabled: false,
                hostEnabled: false,
                level: "off",
    ...
    describe("Factory Configuration", () => {
        it("should return default configurations", () => {
            ...
            assert.ok(defaultConfigs.some((c) => c.type === "posthog"), "Should include PostHog configuration");
            // Restore the stub
    ...
    describe("CLI Subagents Telemetry", () => {
        ...
        it("should respect subagents telemetry category settings", async () => {
            ...
            assert.strictEqual(telemetryService.isCategoryEnabled("subagents"), true, "Subagents category should be enabled by default");
