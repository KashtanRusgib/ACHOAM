 ...
 */
export var TerminalOutputFailureReason;
(function (TerminalOutputFailureReason) {
    TerminalOutputFailureReason["TIMEOUT"] = "timeout";
    TerminalOutputFailureReason["NO_SHELL_INTEGRATION"] = "no_shell_integration";
    TerminalOutputFailureReason["CLIPBOARD_FAILED"] = "clipboard_failed";
})(TerminalOutputFailureReason || (TerminalOutputFailureReason = {}));
/**
 * Enum for terminal user intervention actions
 */
export var TerminalUserInterventionAction;
(function (TerminalUserInterventionAction) {
    TerminalUserInterventionAction["PROCESS_WHILE_RUNNING"] = "process_while_running";
    TerminalUserInterventionAction["MANUAL_PASTE"] = "manual_paste";
    TerminalUserInterventionAction["CANCELLED"] = "cancelled";
})(TerminalUserInterventionAction || (TerminalUserInterventionAction = {}));
/**
 * Enum for terminal hang stages
 */
export var TerminalHangStage;
(function (TerminalHangStage) {
    TerminalHangStage["WAITING_FOR_COMPLETION"] = "waiting_for_completion";
    TerminalHangStage["BUFFER_STUCK"] = "buffer_stuck";
    TerminalHangStage["STREAM_TIMEOUT"] = "stream_timeout";
})(TerminalHangStage || (TerminalHangStage = {}));
/**
...
export class TelemetryService {
     ...
     */
    constructor(providers, telemetryMetadata) {
        ...
        if (hostSetting.isEnabled === Setting.DISABLED) {
            ...
            if (didUserOptIn) {
                void HostProvider.window
                    ...
                    message: "Anonymous Cline error and usage reporting is enabled, but IDE telemetry is disabled. To enable error and usage reporting for this extension, enable telemetry in IDE settings.",
                    options: {
                        items: ["Open Settings"],
                    },
                })
     ...
     */
    captureToProviders(event, properties, required) {
        this.providers.forEach((provider) => {
            try {
                if (required) {
     ...
     */
    captureAuthLoggedOut(provider, reason) {
        this.capture({
     ...
     */
    captureVoiceRecordingStarted(taskId, platform) {
     ...
     */
    captureVoiceRecordingStopped(taskId, durationMs, success, platform) {
        if (!this.isCategoryEnabled("dictation")) {
            return;
        }
        this.capture({
     ...
     */
    captureVoiceTranscriptionStarted(taskId, language) {
        if (!this.isCategoryEnabled("dictation")) {
            return;
        }
        this.capture({
     ...
     */
    captureVoiceTranscriptionCompleted(taskId, transcriptionLength, durationMs, language, isOrgAccount) {
        if (!this.isCategoryEnabled("dictation")) {
            return;
        }
        this.capture({
     ...
     */
    captureVoiceTranscriptionError(taskId, errorType, errorMessage, durationMs) {
        if (!this.isCategoryEnabled("dictation")) {
            return;
        }
        this.capture({
     ...
     */
    captureTaskCreated(ulid, apiProvider, openAiCompatibleDomain) {
     ...
     * @param openAiCompatibleDomain Optional domain for OpenAI Compatible providers (e.g., "api.example.com")
     */
    captureTaskRestarted(ulid, apiProvider, openAiCompatibleDomain) {
        this.resetTaskAggregates(ulid);
        this.capture({
     ...
     */
    captureConversationTurnEvent(ulid, provider = "unknown", model = "unknown", source, mode, tokenUsage = {}, isNativeToolCall) {
     ...
     */
    captureSummarizeTask(ulid, modelId, provider, currentTokens, maxContextWindow) {
        this.capture({
     ...
     */
    captureToolUsage(ulid, tool, modelId, provider, autoApproved, success, workspaceContext, isNativeToolCall = false) {
        this.capture({
     ...
     */
    captureMcpToolCall(ulid, serverName, toolName, status, errorMessage, argumentKeys, isNativeToolCall = false) {
        this.capture({
     ...
     */
    captureDiffEditFailure(ulid, modelId, provider, errorType, isNativeToolCall = false) {
        this.capture({
     ...
     * @param provider The API provider being used
     */
    captureFocusChainIncompleteOnCompletion(ulid, totalItems, completedItems, incompleteItems, modelId, provider) {
        if (!this.isCategoryEnabled("focus_chain")) {
            return;
        }
        this.capture({
     ...
     */
    captureFeatureToggle(ulid, featureName, enabled, modelId) {
        this.capture({
     ...
     */
    captureYoloModeToggle(ulid, enabled) {
        this.capture({
     ...
     */
    captureTerminalExecution(success, method) {
        this.capture({
     ...
     */
    captureTerminalOutputFailure(reason) {
        this.capture({
     ...
     */
    captureTerminalUserIntervention(action) {
        this.capture({
     ...
     */
    captureWorkspaceInitialized(rootCount, vcsTypes, initDurationMs, featureFlagEnabled) {
        this.capture({
     ...
     */
    captureWorkspaceInitError(error, fallbackMode, workspaceCount) {
        this.capture({
     ...
     */
    captureMultiRootCheckpoint(ulid, action, rootCount, successCount, failureCount, durationMs) {
        this.capture({
     ...
     */
    captureWorkspacePathResolved(ulid, context, resolutionType, hintType, resolutionSuccess, targetWorkspaceIndex, isMultiRootEnabled) {
        this.capture({
     ...
     */
    captureWorkspaceSearchPattern(ulid, searchType, workspaceCount, hintProvided, resultsFound, searchDurationMs) {
        this.capture({
     ...
     */
    captureMentionUsed(mentionType, contentLength) {
        this.capture({
     ...
     */
    captureMentionFailed(mentionType, errorType, errorMessage) {
        this.capture({
     ...
     */
    captureMentionSearchResults(query, resultCount, searchType, isEmpty) {
        this.capture({
     ...
     */
    captureSubagentExecution(ulid, durationMs, outputLines, success) {
        if (!this.isCategoryEnabled("subagents")) {
            return;
        }
        this.capture({
    ...
    captureOnboardingProgress(args) {
        this.capture({
