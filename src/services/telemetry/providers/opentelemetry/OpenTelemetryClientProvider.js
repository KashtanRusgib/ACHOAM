...
import { createConsoleLogExporter, createConsoleMetricReader, createOTLPLogExporter, createOTLPMetricReader, } from "./OpenTelemetryExporterFactory";
/**
...
export class OpenTelemetryClientProvider {
        ...
        for (const exporterType of exporters) {
            try {
                ...
                if (exporter) {
                    ...
                    console.log(`[OTEL] Log batch processor configured: maxQueue=${batchConfig.maxQueueSize}, batchSize=${batchConfig.maxExportBatchSize}, timeout=${batchConfig.scheduledDelayMillis}ms`);
                }
            }
            catch (error) {
    ...
    async dispose() {
        ...
        if (this.meterProvider) {
            promises.push(this.meterProvider.shutdown().catch((error) => {
                console.error("Error shutting down MeterProvider:", error);
            }));
        }
        if (this.loggerProvider) {
            promises.push(this.loggerProvider.shutdown().catch((error) => {
...
//# sourceMappingURL=OpenTelemetryClientProvider.js.map