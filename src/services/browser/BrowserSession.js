...
export class BrowserSession {
    ...
    ulid;
    stateManager;
    constructor(stateManager, useWebp = true) {
    ...
    async launchBrowser() {
        ...
        if (browserSettings.remoteBrowserEnabled) {
            ...
            try {
                ...
                if (this.ulid) {
                    telemetryService.captureBrowserError(this.ulid, "remote_browser_launch_error", error instanceof Error ? error.message : String(error), {
                        isRemote: true,
    ...
    async launchRemoteBrowser() {
        ...
        if (browserWSEndpoint && Date.now() - this.lastConnectionAttempt < 3600000) {
            try {
                ...
                // Capture error telemetry
                if (this.ulid) {
                    telemetryService.captureBrowserError(this.ulid, "cached_endpoint_connection_error", error instanceof Error ? error.message : String(error), {
                        isRemote: true,
        ...
        if (remoteBrowserHost) {
            try {
                ...
                // Capture error telemetry
                if (this.ulid) {
                    telemetryService.captureBrowserError(this.ulid, "remote_host_connection_error", error instanceof Error ? error.message : String(error), {
                        isRemote: true,
                        remoteBrowserHost,
                    });
                }
