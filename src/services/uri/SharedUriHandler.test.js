...
describe("SharedUriHandler", () => {
    let sandbox;
    let handleOpenRouterCallbackStub;
    let handleAuthCallbackStub;
    beforeEach(async () => {
        ...
        sandbox.stub(ErrorService, "initialize").resolves(mockErrorService);
        sandbox.stub(ErrorService, "get").returns(mockErrorService);
    ...
    describe("handleUri", () => {
        ...
        describe("Query parameter parsing", () => {
            it("should correctly parse multiple query parameters", async () => {
                const result = await SharedUriHandler.handleUri("vscode://cline.cline/auth?idToken=jwt123&provider=github&extra=param");
                expect(result).to.be.true;
                sinon.assert.calledOnceWithExactly(handleAuthCallbackStub, "jwt123", "github");
            });
            it("should handle URL-encoded parameters", async () => {
                const result = await SharedUriHandler.handleUri("vscode://cline.cline/auth?idToken=jwt%20with%20spaces&provider=google");
                expect(result).to.be.true;
...
//# sourceMappingURL=SharedUriHandler.test.js.map