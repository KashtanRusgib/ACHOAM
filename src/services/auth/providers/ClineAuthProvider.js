...
export class ClineAuthProvider {
     ...
     */
    async shouldRefreshIdToken(_refreshToken, expiresAt) {
        try {
        ...
        try {
            ...
            if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
                ...
                if (timeSinceLastAttempt < this.RETRY_DELAY_MS && this.refreshRetryCount > 0) {
                    Logger.debug(`Waiting ${Math.ceil((this.RETRY_DELAY_MS - timeSinceLastAttempt) / 1000)}s before retry attempt ${this.refreshRetryCount + 1}/${this.MAX_REFRESH_RETRIES}`);
                    return null;
                }
                ...
                Logger.debug(`Token expired or expiring soon, attempting refresh (attempt ${this.refreshRetryCount}/${this.MAX_REFRESH_RETRIES}). API Base URL: ${this.config.apiBaseUrl}`);
                try {
                    ...
                    Logger.error(`Token refresh failed (attempt ${this.refreshRetryCount}/${this.MAX_REFRESH_RETRIES}):`, refreshError);
    ...
    async getAuthRequest(callbackUrl) {
    ...
    async signIn(controller, authorizationCode, provider) {
        try {
            ...
            const responseType = responseJSON;
    ...
    async fetchRemoteUserInfo(tokenData) {
        try {
...
//# sourceMappingURL=ClineAuthProvider.js.map