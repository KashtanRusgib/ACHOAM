...
export class AuthService {
    ...
    _clineAuthInfo = null;
    _provider = null;
    _activeAuthStatusUpdateHandlers = new Set();
    _handlerToController = new Map();
    _controller;
    ...
    async createAuthRequest(strict = false) {
    ...
    async handleDeauth(reason = LogoutReason.UNKNOWN) {
        if (!this._provider) {
            throw new Error("Auth provider is not set");
        }
        try {
    ...
    async handleAuthCallback(authorizationCode, provider) {
        if (!this._provider) {
            throw new Error("Auth provider is not set");
        }
        try {
