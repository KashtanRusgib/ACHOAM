...
export function isNextGenModelProvider(providerInfo) {
...
export function isClaude4PlusModelFamily(id) {
...
export function isGPT51Model(id) {
    const modelId = normalize(id);
    return modelId.includes("gpt-5.1") || modelId.includes("gpt-5-1");
}
export function isGLMModelFamily(id) {
    const modelId = normalize(id);
    return (modelId.includes("glm-4.6") ||
        modelId.includes("glm-4.5") ||
        modelId.includes("z-ai/glm") ||
        modelId.includes("zai-org/glm"));
}
export function isMinimaxModelFamily(id) {
    const modelId = normalize(id);
    return modelId.includes("minimax");
}
export function isHermesModelFamily(id) {
    const modelId = normalize(id);
    return (modelId.includes("hermes-4") ||
...
export function isNextGenOpenSourceModelFamily(id) {
    const modelId = normalize(id);
    return ["kimi-k2"].some((substring) => modelId.includes(substring));
}
export function isMicrowaveModelFamily(id) {
    const modelId = normalize(id);
    return modelId.includes("microwave");
}
export function isGemini3ModelFamily(id) {
    const modelId = normalize(id);
    return modelId.includes("gemini3") || modelId.includes("gemini-3");
}
function isDeepSeek32ModelFamily(id) {
    const modelId = normalize(id);
