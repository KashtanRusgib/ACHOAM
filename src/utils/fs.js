...
export const readDirectory = async (directoryPath, excludedPaths = []) => {
    try {
        const filePaths = await fs
            ...
            const resolvedPath = workspaceResolver.resolveWorkspacePath(file.parentPath, file.name, "Utils.fs.readDirectory");
...
export async function getBinaryLocation(name) {
