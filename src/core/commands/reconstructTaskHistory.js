 ...
 */
export async function reconstructTaskHistory(showNotifications = true) {
    try {
        // Show confirmation dialog using HostProvider
        const proceed = await HostProvider.window.showMessage({
            type: ShowMessageType.WARNING,
            message: "This will rebuild your task history from existing task data. This operation will backup your current task history and attempt to reconstruct it from task folders. Continue?",
            options: {
...
async function performTaskHistoryReconstruction() {
    const result = {
        totalTasks: 0,
...
async function backupExistingTaskHistory() {
    try {
...
async function scanTaskDirectories(tasksDir) {
    const fs = await import("fs/promises");
    try {
...
async function reconstructTaskHistoryItem(taskId) {
    try {
...
function extractTaskInformation(clineMessages, metadata) {
...
//# sourceMappingURL=reconstructTaskHistory.js.map