...
import { SapAiCoreModelDeployment, SapAiCoreModelsResponse } from "@/shared/proto/cline/models"
/**
 ...
 */
async function fetchAiCoreDeploymentsAndOrchestration(accessToken, baseUrl, resourceGroup) {
	if (!accessToken) {
	...
	try {
		...
		const runningDeployments = allDeployments.filter((deployment) => deployment.targetStatus === "RUNNING")
