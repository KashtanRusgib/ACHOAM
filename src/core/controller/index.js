...
export class Controller {
	...
	constructor(context) {
		...
		const authStatusHandler = async (response, _isLast, _seqNumber) => {
			if (response.user) {
				fetchRemoteConfig(this)
			}
		}
    ...
    */
	...
	async toggleActModeForYoloMode() {
		const modeToSwitchTo = "act"
		// Switch to act mode
	...
	async handleOcaAuthCallback(code, state) {
		try {
			await this.ocaAuthService.handleAuthCallback(code, state)
			const ocaProvider = "oca"
			// Get current settings to determine how to update providers
			const planActSeparateModelsSetting = this.stateManager.getGlobalSettingsKey("planActSeparateModelsSetting")
			const currentMode = this.stateManager.getGlobalSettingsKey("mode")
			// Get current API configuration from cache
	...
	async handleMcpOAuthCallback(serverHash, code, state) {
		try {
	...
	async handleTaskCreation(prompt) {
		await sendChatButtonClickedEvent()
		await this.initTask(prompt)
	}
	// MCP Marketplace
	async fetchMcpMarketplaceFromApi() {
	...
	async refreshMcpMarketplace(sendCatalogEvent) {
		try {
    ...
    */
	 ...
	 */
	async trackBannerEvent(bannerId, eventType) {
		try {
			await this.ensureBannerService()
			if (BannerService.isInitialized()) {
