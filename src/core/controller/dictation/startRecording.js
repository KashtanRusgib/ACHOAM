 ...
 */
async function handleInstallWithCline(controller, dependencyName, installCommand, platform) {
 ...
 */
async function handleCopyCommand(installCommand) {
 ...
 */
async function handleMissingDependency(controller, platform, config) {
 ...
 */
async function handleSignInError(controller, errorMessage) {
 ...
 */
async function showGenericError(errorMessage) {
	await HostProvider.window.showMessage({
		type: ShowMessageType.ERROR,
		message: `Voice recording error: ${errorMessage}`,
		options: { items: [] },
 ...
 */
function isMissingDependencyError(error, config) {
 ...
 */
export const startRecording = async (controller) => {
