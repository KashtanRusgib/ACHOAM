...
import { condenseToolResponse, deepPlanningToolResponse, explainChangesToolResponse, newRuleToolResponse, newTaskToolResponse, reportBugToolResponse, subagentToolResponse, } from "../prompts/commands";
import { StateManager } from "../storage/StateManager";
/**
 * Processes text for slash commands and transforms them with appropriate instructions
 * This is called after parseMentions() to process any slash commands in the user's message
 */
export async function parseSlashCommands(text, localWorkflowToggles, globalWorkflowToggles, ulid, focusChainSettings, enableNativeToolCalls, providerInfo) {
    ...
    const removeSlashCommand = (fullText, _tagContent, // kept for clarity about the context
    contentStartIndex, slashMatch) => {
    ...
    for (const { regex } of tagPatterns) {
        ...
        if (tagMatch) {
            ...
            const enabledRemoteWorkflows = remoteWorkflows
                .filter((workflow) => {
            ...
            const enabledWorkflows = [...localWorkflows, ...globalWorkflows, ...enabledRemoteWorkflows];
