...
import { getGlobalClineRules, getLocalClineRules, refreshClineRulesToggles, } from "@core/context/instructions/user-instructions/cline-rules";
import { getLocalAgentsRules, getLocalCursorRules, getLocalWindsurfRules, refreshExternalRulesToggles, } from "@core/context/instructions/user-instructions/external-rules";
...
import { ensureRulesDirectoryExists, ensureTaskDirectoryExists, GlobalFileNames, getSavedApiConversationHistory, getSavedClineMessages, } from "@core/storage/disk";
...
export class Task {
    // Constants
    static STANDALONE_TERMINAL_MODULE_PATH = path.join(__dirname, "../standalone/runtime-files/vscode/enhanced-terminal.js");
    // Core task variables
    taskId;
    ulid;
    taskIsFavorited;
     ...
     */
    async setActiveHookExecution(hookExecution) {
     ...
     * PUBLIC: Exposed for ToolExecutor to use
     */
    async clearActiveHookExecution() {
        await this.withStateLock(() => {
    ...
    taskLockAcquired;
    constructor(params) {
        const { controller, mcpHub, updateTaskHistory, postStateToWebview, reinitExistingTaskFromId, cancelTask, shellIntegrationTimeout, terminalReuseEnabled, terminalOutputLineLimit, subagentTerminalOutputLineLimit, defaultTerminalProfile, vscodeTerminalExecutionMode, cwd, stateManager, workspaceManager, task, images, files, historyItem, taskId, taskLockAcquired, } = params;
        ...
        const storedApiConfigurations = this.stateManager.getApiConfigurations?.();
        ...
        const achoamHeads = normalizedConfigs.map((bot) => ({
            id: bot.id,
        ...
        const headsToUse = achoamHeads.length > 0
            ? achoamHeads
            : [
                {
                    id: "default-head",
    ...
    async saveCheckpointCallback(isAttemptCompletionMessage, completionMessageTs) {
     ...
     */
    async handleHookCancellation(hookName, wasCancelled) {
    ...
    async runUserPromptSubmitHook(userContent, _context) {
    ...
    async resumeTaskFromHistory() {
        ...
        const mode = this.stateManager.getGlobalSettingsKey("mode");
        const [taskResumptionMessage, userResponseMessage] = formatResponse.taskResumption(mode === "plan" ? "plan" : "act", agoText, this.cwd, wasRecent, responseText, hasPendingFileContextWarnings);
    ...
    async shouldRunTaskCancelHook() {
        ...
        const clineMessages = this.messageStateHandler.getClineMessages();
        const lastMessage = clineMessages.at(-1);
        const isAtButtonOnlyState = lastMessage?.type === "ask" &&
    ...
    async abortTask() {
        try {
            ...
            if (this.FocusChainManager) {
                ...
                const currentProvider = (currentMode === "plan" ? apiConfig.planModeApiProvider : apiConfig.actModeApiProvider);
    ...
    async executeCommandTool(command, timeoutSeconds) {
        ...
        if (this.terminalExecutionMode === "backgroundExec") {
            this.activeBackgroundCommand = { process: process, command, outputLines: [] };
        }
        ...
        const result = terminalManager.processOutput(outputLines, isSubagent ? terminalManager["subagentTerminalOutputLineLimit"] : undefined, isSubagent);
        if (didCancelViaUi) {
            return [
                true,
                formatResponse.toolResult(`Command cancelled. ${result.length > 0 ? `\nOutput captured before cancellation:\n${result}` : ""}`),
            ];
        }
    ...
    getApiRequestIdSafe() {
        const apiLike = this.api;
        return apiLike.getLastRequestId?.() ?? apiLike.lastGenerationId;
    }
    async handleContextWindowExceededError() {
        const apiConversationHistory = this.messageStateHandler.getApiConversationHistory();
        this.taskState.conversationHistoryDeletedRange = this.contextManager.getNextTruncationRange(apiConversationHistory, this.taskState.conversationHistoryDeletedRange, "quarter");
        await this.messageStateHandler.saveClineMessagesAndUpdateHistory();
        await this.contextManager.triggerApplyStandardContextTruncationNoticeChange(Date.now(), await ensureTaskDirectoryExists(this.taskId), apiConversationHistory);
    ...
    async *attemptApiRequest(previousApiReqIndex) {
        ...
        const preferredLanguage = getLanguageKey(preferredLanguageRaw);
        ...
        const { windsurfLocalToggles, cursorLocalToggles, agentsLocalToggles } = await refreshExternalRulesToggles(this.controller, this.cwd);
        ...
        if (multiRootEnabled && this.workspaceManager) {
            workspaceRoots = this.workspaceManager.getRoots().map((root) => ({
                ...
                vcs: root.vcs, // Cast VcsType to string
            }));
        }
        ...
        try {
            ...
            if (isContextWindowExceededError && !this.taskState.didAutomaticallyRetryFailedApiRequest) {
                ...
                if (!isClineProviderInsufficientCredits && !isAuthError && this.taskState.autoRetryAttempts < 3) {
                    ...
                    await this.say("error_retry", JSON.stringify({
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        if (isFirstRequest && this.stateManager.getGlobalSettingsKey("enableCheckpointsSetting") && this.checkpointManager) {
            ...
            if (lastCheckpointMessageIndex !== -1) {
                this.checkpointManager
                    ...
                    .catch((error) => {
                    console.error(`[TaskCheckpointManager] Failed to create checkpoint commit for task ${this.taskId}:`, error);
                });
            }
        }
        ...
        let [parsedUserContent, environmentDetails, clinerulesError] = await this.loadContext(userContent, includeFileDetails, useCompactPrompt);
        ...
        if (useAutoCondense && isNextGenModelFamily(this.api.getModel().id)) {
            ...
            if (this.taskState.currentlySummarizing) {
                ...
                shouldCompact = this.contextManager.shouldCompactContextWindow(this.messageStateHandler.getClineMessages(), this.api, previousApiReqIndex, autoCondenseThreshold);
                ...
                if (shouldCompact) {
                    shouldCompact = await this.contextManager.attemptFileReadOptimization(this.messageStateHandler.getApiConversationHistory(), this.taskState.conversationHistoryDeletedRange, this.messageStateHandler.getClineMessages(), previousApiReqIndex, await ensureTaskDirectoryExists(this.taskId));
                }
            }
        ...
        if (shouldCompact) {
            userContent.push({
                type: "text",
                text: summarizeTask(this.stateManager.getGlobalSettingsKey("focusChainSettings"), this.cwd, isMultiRootEnabled(this.stateManager)),
            });
        }
        ...
        if (isFirstRequest) {
            ...
            telemetryService.captureTaskInitialization(this.ulid, this.taskId, durationMs, this.stateManager.getGlobalSettingsKey("enableCheckpointsSetting"));
        }
        ...
        try {
            ...
            const abortStream = async (cancelReason, streamingFailedMessage) => {
                ...
                telemetryService.captureConversationTurnEvent(this.ulid, providerId, modelInfo.modelId, "assistant", modelInfo.mode, {
            ...
            try {
                for await (const chunk of stream) {
                    ...
                    await this.presentAssistantMessage().catch((error) => Logger.debug("[Task] Failed to present message: " + error));
                    if (this.taskState.abort) {
                        this.api.abort?.();
            ...
            if (assistantHasContent) {
                telemetryService.captureConversationTurnEvent(this.ulid, providerId, model.id, "assistant", modelInfo.mode, {
                    tokensIn: taskMetrics.inputTokens,
                    tokensOut: taskMetrics.outputTokens,
                    cacheWriteTokens: taskMetrics.cacheWriteTokens,
                    cacheReadTokens: taskMetrics.cacheReadTokens,
            ...
            if (assistantHasContent) {
                ...
                if (this.taskState.autoRetryAttempts < 3) {
                    ...
                    const askResult = await this.ask("api_req_failed", "No assistant message was received. Would you like to retry the request?");
                    response = askResult.response;
    ...
    async loadContext(userContent, includeFileDetails = false, useCompactPrompt = false) {
        ...
        const hasUserContentTag = (text) => {
            return USER_CONTENT_TAGS.some((tag) => text.includes(tag));
        };
        const parseTextBlock = async (text) => {
            const parsedText = await parseMentions(text, cwd, this.urlContentFetcher, this.fileContextTracker, this.workspaceManager);
            const { processedText, needsClinerulesFileCheck: needsCheck } = await parseSlashCommands(parsedText, localWorkflowToggles, globalWorkflowToggles, ulid, focusChainSettings, useNativeToolCalls, providerInfo);
            if (needsCheck) {
                needsClinerulesFileCheck = true;
            }
            return processedText;
        };
        const processTextContent = async (block) => {
        ...
        const processContentBlock = async (block) => {
            if (block.type === "text") {
            ...
            if (block.type === "tool_result") {
                ...
                if (Array.isArray(block.content)) {
                    const processedContent = await Promise.all(block.content.map(async (contentBlock) => {
    ...
    processNativeToolCalls(assistantTextOnly, toolBlocks) {
        ...
        const textBlocks = textContent ? [{ type: "text", content: textContent, partial: false }] : [];
     ...
     */
    getPrimaryWorkspaceName(primary) {
        if (primary?.name) {
            return primary.name;
        }
        if (primary?.path) {
    ...
    async getEnvironmentDetails(includeFileDetails = false) {
        ...
        if (isNextGenModel) {
            const autoCondenseThreshold = this.stateManager.getGlobalSettingsKey("autoCondenseThreshold") ?? 0.75;
