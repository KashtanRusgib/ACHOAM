...
export async function showChangedFilesDiff(messageStateHandler, checkpointTracker, messageTs, seeNewChangesSinceLastTaskCompletion) {
    ...
    const changedFiles = await getChangedFiles(messageStateHandler, checkpointTracker, seeNewChangesSinceLastTaskCompletion, messageIndex, lastCheckpointHash);
...
async function getChangedFiles(messageStateHandler, checkpointTracker, changesSinceLastTaskCompletion, messageIndex, lastCheckpointHash) {
    try {
        let changedFiles;
        if (changesSinceLastTaskCompletion) {
            changedFiles = await getChangesSinceLastTaskCompletion(messageStateHandler, checkpointTracker, messageIndex, lastCheckpointHash);
        }
        else {
...
async function getChangesSinceLastTaskCompletion(messageStateHandler, checkpointTracker, messageIndex, lastCheckpointHash) {
    ...
    const lastTaskCompletedMessageCheckpointHash = findLast(messageStateHandler.getClineMessages().slice(0, messageIndex), (m) => m.say === "completion_result")?.lastCheckpointHash; // ask is only used to relinquish control, its the last say we care about
