...
export class StreamResponseHandler {
    toolUseHandler = new ToolUseHandler();
    reasoningHandler = new ReasoningHandler();
    _requestId;
    setRequestId(id) {
...
class ToolUseHandler {
    ...
    processToolUseDelta(delta, call_id) {
    ...
    getAllFinalizedToolUses(summary) {
        const results = [];
    ...
    createPendingToolUse(id, name, call_id) {
    ...
    extractPartialJsonFields(partialJson) {
        const result = {};
        const pattern = /"(\w+)":\s*"((?:[^"\\]|\\.)*)(?:")?/g;
...
//# sourceMappingURL=StreamResponseHandler.js.map