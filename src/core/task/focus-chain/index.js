...
import { createFocusChainMarkdownContent, extractFocusChainItemsFromText, extractFocusChainListFromText, getFocusChainFilePath, } from "./file-utils";
...
export class FocusChainManager {
            ...
            try {
                ...
                if (markdownTodoList) {
                    ...
                    if (previousList !== markdownTodoList) {
                        ...
                        console.log(`[Task ${this.taskId}] Focus Chain List: File watcher triggered but content unchanged, skipping update`);
                    }
                }
            }
            catch (error) {
    ...
    async updateFCListFromToolResponse(taskProgress) {
        try {
            ...
            if (taskProgress && taskProgress.trim()) {
                ...
                console.debug(`[Task ${this.taskId}] focus chain list: LLM provided focus chain list update via task_progress parameter. Length ${previousList?.length || 0} > ${this.taskState.currentFocusChainChecklist.length}`);
    ...
    shouldIncludeFocusChainInstructions() {
        ...
        const reachedReminderInterval = this.taskState.apiRequestsSinceLastTodoUpdate >= this.focusChainSettings.remindClineInterval;
        ...
        const shouldInclude = reachedReminderInterval ||
     ...
     */
    checkIncompleteProgressOnCompletion(modelId, provider) {
        if (this.focusChainSettings.enabled && this.taskState.currentFocusChainChecklist) {
            ...
            if (totalItems > 0 && completedItems < totalItems) {
                ...
                telemetryService.captureFocusChainIncompleteOnCompletion(this.taskId, totalItems, completedItems, incompleteItems, modelId, provider);
            }
        }
    }
    /**
