...
export class ToolResultUtils {
     ...
     */
    static pushToolResult(content, block, userMessageContent, toolDescription, _api, markToolAsUsed, coordinator, toolUseIdMap) {
        if (typeof content === "string") {
            ...
            if (userMessageContent.some((item) => item.type === "tool_result" && item.tool_use_id === toolUseId && item.content)) {
    ...
    static createToolResultBlock(content, id, call_id) {
     ...
     */
    static async askApprovalAndPushFeedback(type, completeMessage, config) {
