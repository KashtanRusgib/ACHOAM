...
export class SearchFilesToolHandler {
     ...
     */
    determineSearchPaths(config, parsedPath, workspaceHint, originalPath) {
     ...
     */
    async executeSearch(config, absolutePath, workspaceName, workspaceRoot, regex, filePattern) {
        try {
            // Use workspace root for relative path calculation, fallback to cwd
            const basePathForRelative = workspaceRoot || config.cwd;
            const workspaceResults = await regexSearchFiles(basePathForRelative, absolutePath, regex, filePattern, config.services.clineIgnoreController);
     ...
     */
    formatSearchResults(config, searchResults, searchPaths) {
        const allResults = [];
    ...
    async execute(config, block) {
        ...
        const resolvedToNonPrimary = searchPaths.length === 0
            ? true
        ...
        const workspaceContext = {
            ...
            resolutionMethod: (workspaceHint ? "hint" : searchPaths.length > 1 ? "path_detection" : "primary_fallback"),
        };
        ...
        if (config.isMultiRootEnabled && config.workspaceManager) {
            const resolutionType = workspaceHint
                ...
                : searchPaths.length > 1
                    ...
                    : "fallback_to_primary";
            telemetryService.captureWorkspacePathResolved(config.ulid, "SearchFilesToolHandler", resolutionType, workspaceHint ? "workspace_name" : undefined, searchPaths.length > 0, // resolution success = found paths to search
            undefined, // TODO: could calculate primary workspace index
            true);
        }
        // Execute searches in all relevant workspaces in parallel
        const searchPromises = searchPaths.map(({ absolutePath, workspaceName, workspaceRoot }) => this.executeSearch(config, absolutePath, workspaceName, workspaceRoot, regex, filePattern));
        ...
        if (config.isMultiRootEnabled && config.workspaceManager) {
            ...
            telemetryService.captureWorkspaceSearchPattern(config.ulid, searchType, searchPaths.length, !!workspaceHint, resultsFound, searchDurationMs);
        }
        const sharedMessageProps = {
            tool: "searchFiles",
