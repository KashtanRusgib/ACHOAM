...
export class AttemptCompletionHandler {
    ...
    async handlePartialBlock(block, uiHelpers) {
        ...
        if (!command) {
            ...
            await uiHelpers.say("completion_result", uiHelpers.removeClosingTag(block, "result", result), undefined, undefined, block.partial);
        }
    }
    async execute(config, block) {
        ...
        const clineMessages = config.messageState.getClineMessages();
        const partialCompletionIndex = findLastIndex(clineMessages, (m) => m.partial === true && m.type === "say" && m.say === "completion_result");
     ...
     */
    async runTaskCompleteHook(config, block) {
