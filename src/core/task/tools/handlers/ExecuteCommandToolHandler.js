...
export class ExecuteCommandToolHandler {
    ...
    async handlePartialBlock(block, uiHelpers) {
        ...
        if (shouldAutoApprove) {
            ...
            await uiHelpers
                .ask("command", uiHelpers.removeClosingTag(block, "command", command), block.partial)
                .catch(() => { });
        }
    }
    async execute(config, block) {
        ...
        const timeoutParam = block.params.timeout;
        let timeoutSeconds;
        ...
        const workspaceContext = {
            ...
            resolutionMethod: (workspaceHintUsed ? "hint" : "primary_fallback"),
        };
        // Capture workspace path resolution telemetry
        if (config.isMultiRootEnabled && config.workspaceManager) {
            telemetryService.captureWorkspacePathResolved(config.ulid, "ExecuteCommandToolHandler", workspaceHintUsed ? "hint_provided" : "fallback_to_primary", workspaceHintUsed ? "workspace_name" : undefined, resolvedToNonPrimary, // resolution success = resolved to different workspace
        ...
        if ((!requiresApprovalPerLLM && autoApproveSafe) || (requiresApprovalPerLLM && autoApproveSafe && autoApproveAll)) {
            ...
            showNotificationForApproval(`Cline wants to execute a command: ${actualCommand}`, config.autoApprovalSettings.enableNotifications);
            const didApprove = await ToolResultUtils.askApprovalAndPushFeedback("command", actualCommand + `${autoApproveSafe && requiresApprovalPerLLM ? COMMAND_REQ_APP_STRING : ""}`, config);
            if (!didApprove) {
