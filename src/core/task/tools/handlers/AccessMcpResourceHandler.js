...
export class AccessMcpResourceHandler {
    name = ClineDefaultTool.MCP_ACCESS;
    ...
    async execute(config, block) {
        ...
        if (shouldAutoApprove) {
            ...
            // Capture telemetry
            telemetryService.captureToolUsage(config.ulid, block.name, config.api.getModel().id, provider, true, true, undefined, block.isNativeToolCall);
        }
        else {
