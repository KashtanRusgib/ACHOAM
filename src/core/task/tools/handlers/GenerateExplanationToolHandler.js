import { buildDiffContent, isBinaryFile, openDiffView, setupCommentController, streamAIExplanationComments, stringifyConversationHistory, } from "@core/controller/task/explainChangesShared";
 ...
 */
function createExplanationMessage(title, fromRef, toRef, status, error) {
    const message = { title, fromRef, toRef, status };
    if (error) {
        message.error = error;
    }
    return JSON.stringify(message);
}
export class GenerateExplanationToolHandler {
    name = ClineDefaultTool.GENERATE_EXPLANATION;
    ...
    async execute(config, block) {
        ...
        const fromRef = block.params.from_ref;
        const toRef = block.params.to_ref; // Optional - if not provided, compare to working directory
        ...
        await config.callbacks.say("generate_explanation", createExplanationMessage(title, fromRef, toRefDisplay, "generating"), undefined, undefined, true);
        // Get API configuration
        const apiConfiguration = config.services.stateManager.getApiConfiguration();
        if (!apiConfiguration) {
            await config.callbacks.say("generate_explanation", createExplanationMessage(title, fromRef, toRefDisplay, "error", "API configuration not available"), undefined, undefined, false);
        ...
        try {
            ...
            if (!isRepo) {
                ...
                await config.callbacks.say("generate_explanation", createExplanationMessage(title, fromRef, toRefDisplay, "error", errorMsg), undefined, undefined, false);
            ...
            if (changedFiles.length === 0) {
                return formatResponse.toolResult(`All changed files between '${fromRef}' and '${toRef || "working directory"}' are binary files that cannot be displayed.`);
            }
            ...
            const commentCount = await streamAIExplanationComments(apiConfiguration, diffContent, `${title}\n\n${conversationContext}`, changedFiles, 
            ...
            (filePath, startLine, endLine) => {
                ...
                commentController.startStreamingComment(filePath, startLine, endLine, matchingFile?.relativePath, matchingFile?.after, shouldRevealComments);
            }, 
            ...
            await config.callbacks.say("generate_explanation", createExplanationMessage(title, fromRef, toRefDisplay, "complete"), undefined, undefined, false);
            const refDescription = toRef ? `'${fromRef}' and '${toRef}'` : `'${fromRef}' and working directory`;
            return formatResponse.toolResult(`Successfully generated ${commentCount} explanation comment${commentCount === 1 ? "" : "s"} for ${changedFiles.length} changed file${changedFiles.length === 1 ? "" : "s"} between ${refDescription}. The diff view is now open with inline explanations.`);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : "Unknown error";
            console.error("Error in generate_explanation:", errorMessage);
            await config.callbacks.say("generate_explanation", createExplanationMessage(title, fromRef, toRefDisplay, "error", `Failed to generate explanations: ${errorMessage}`), undefined, undefined, false);
            return formatResponse.toolError(`Failed to generate explanations: ${errorMessage}`);
        }
    }
}
//# sourceMappingURL=GenerateExplanationToolHandler.js.map