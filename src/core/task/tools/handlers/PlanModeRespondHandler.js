...
export class PlanModeRespondHandler {
    ...
    async execute(config, block) {
        ...
        if (needsMoreExploration) {
            return formatResponse.toolResult(`[You have indicated that you need more exploration. Proceed with calling tools to continue the planning process.]`);
        }
        ...
        if (config.mode === "plan" && config.yoloModeToggled && !needsMoreExploration) {
            ...
            if (switchSuccessful) {
                ...
                const lastPlanMessage = findLast(config.messageState.getClineMessages(), (m) => m.ask === this.name);
                if (lastPlanMessage) {
        ...
        let { text, images, files: planResponseFiles, } = await config.callbacks.ask(this.name, JSON.stringify(sharedMessage), false);
        ...
        if (config.taskState.didRespondToPlanAskBySwitchingMode) {
            const result = formatResponse.toolResult(`[The user has switched to ACT MODE, so you may now proceed with the task.]` +
                (text
                    ? `\n\nThe user also provided the following message when switching to ACT MODE:\n<user_message>\n${text}\n</user_message>`
                    : ""), images, fileContentString);
