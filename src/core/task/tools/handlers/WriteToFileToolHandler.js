...
export class WriteToFileToolHandler {
    validator;
    name = ClineDefaultTool.FILE_NEW; // This handler supports write_to_file, replace_in_file, and new_rule
    ...
    async handlePartialBlock(block, uiHelpers) {
        ...
        try {
            ...
            const sharedMessageProps = {
                ...
                path: getReadablePath(config.cwd, uiHelpers.removeClosingTag(block, block.params.path ? "path" : "absolutePath", relPath)),
    ...
    async execute(config, block) {
        ...
        if (!rawRelPath) {
            ...
            await config.services.diffViewProvider.reset();
            return await config.callbacks.sayAndCreateMissingParamError(block.name, block.params.absolutePath ? "absolutePath" : "path");
        }
        ...
        try {
            ...
            if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
                ...
                // Capture telemetry
                telemetryService.captureToolUsage(config.ulid, block.name, modelId, providerId, true, true, workspaceContext, block.isNativeToolCall);
                ...
                if (response !== "yesButtonClicked") {
                    ...
                    telemetryService.captureToolUsage(config.ulid, block.name, modelId, providerId, false, false, workspaceContext, block.isNativeToolCall);
                    await config.services.diffViewProvider.revertChanges();
                    ...
                    telemetryService.captureToolUsage(config.ulid, block.name, modelId, providerId, false, true, workspaceContext, block.isNativeToolCall);
                }
            }
            ...
            const { newProblemsMessage, userEdits, autoFormattingEdits, finalContent } = await config.services.diffViewProvider.saveChanges();
            ...
            if (userEdits) {
                ...
                await config.callbacks.say("user_feedback_diff", JSON.stringify({
                    tool: fileExists ? "editedExistingFile" : "newFileCreated",
                    path: relPath,
                    diff: userEdits,
                }));
                return formatResponse.fileEditWithUserChanges(relPath, userEdits, autoFormattingEdits, finalContent, newProblemsMessage);
            }
            else {
     ...
     */
    async validateAndPrepareFileOperation(config, block, relPath, diff, content) {
        ...
        if (!accessValidation.ok) {
            ...
            ToolResultUtils.pushToolResult(errorResponse, block, config.taskState.userMessageContent, ToolDisplayUtils.getToolDescription, config.api, () => {
                config.taskState.didAlreadyUseTool = true;
            }, config.coordinator, config.taskState.toolUseIdMap);
            return;
        }
        ...
        if (diff) {
            ...
            try {
                newContent = await constructNewFileContent(diff, config.services.diffViewProvider.originalContent || "", !block.partial);
            }
            catch (error) {
                ...
                const errorResponse = formatResponse.toolError(`${error?.message}\n\n` +
