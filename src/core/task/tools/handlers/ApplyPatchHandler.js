...
import { BASH_WRAPPERS, DiffError, PATCH_MARKERS, PatchActionType } from "@/shared/Patch";
...
export class ApplyPatchHandler {
    ...
    async previewPatchStream(rawInput, uiHelpers) {
        ...
        const requiresOpen = !provider.isEditing || state.currentPreviewPath !== targetResolution.resolvedPath || provider.editType === undefined;
        ...
        const stream = { content: undefined };
        switch (actionType) {
            ...
            case PatchActionType.UPDATE: {
                const sourceResolution = await this.pathResolver.resolveAndValidate(targetPath, "ApplyPatchHandler.previewPatch.source");
                if (!sourceResolution) {
                    return;
                }
    ...
    isPatchLine(line) {
        return (line.startsWith(PATCH_MARKERS.ADD) ||
    ...
    extractFilesForOperations(text, markers) {
    ...
    patchToCommit(patch, originalFiles) {
        const changes = {};
     ...
     */
    applyChunks(content, chunks, path, tryPreserveEscaping = false) {
        if (chunks.length === 0) {
            return content;
        }
    ...
    async generateChangeSummary(changes) {
        const summaries = await Promise.all(Object.entries(changes).map(async ([file, change]) => {
    ...
    async handleApproval(config, block, message, rawInput) {
        ...
        if (shouldAutoApprove) {
            ...
            telemetryService.captureToolUsage(config.ulid, this.name, modelId, providerId, true, true, undefined, block.isNativeToolCall);
            return true;
        }
        ...
        telemetryService.captureToolUsage(config.ulid, this.name, modelId, providerId, false, approved, undefined, block.isNativeToolCall);
        return approved;
    }
}
//# sourceMappingURL=ApplyPatchHandler.js.map