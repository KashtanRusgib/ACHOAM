...
export class SummarizeTaskHandler {
    ...
    async execute(config, block) {
        try {
            ...
            config.taskState.conversationHistoryDeletedRange = config.services.contextManager.getNextTruncationRange(apiConversationHistory, config.taskState.conversationHistoryDeletedRange, keepStrategy);
            await config.messageState.saveClineMessagesAndUpdateHistory();
            await config.services.contextManager.triggerApplyStandardContextTruncationNoticeChange(Date.now(), await ensureTaskDirectoryExists(config.taskId), apiConversationHistory);
            // Set summarizing state
            config.taskState.currentlySummarizing = true;
            // Capture telemetry after main business logic is complete
            const telemetryData = config.services.contextManager.getContextTelemetryData(config.messageState.getClineMessages(), config.api, config.taskState.lastAutoCompactTriggerIndex);
            if (telemetryData) {
                ...
                telemetryService.captureSummarizeTask(config.ulid, config.api.getModel().id, provider, telemetryData.tokensUsed, telemetryData.maxContextWindow);
            }
            return toolResult;
        }
        catch (error) {
