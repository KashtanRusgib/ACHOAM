 ...
 */
export async function getStandaloneLockManager() {
    try {
 ...
 */
export async function tryAcquireFolderLockWithRetry(options, config) {
 ...
 */
export async function releaseFolderLock(taskId, lockTarget) {
    try {
        const lockManager = await getStandaloneLockManager();
        if (!lockManager) {
 ...
 */
export async function acquireFolderLock(options) {
    const lockManager = await getStandaloneLockManager();
    if (!lockManager) {
 ...
 */
export async function retryFolderLockAcquisition(operation, config = DEFAULT_RETRY_CONFIG) {
...
//# sourceMappingURL=FolderLockUtils.js.map