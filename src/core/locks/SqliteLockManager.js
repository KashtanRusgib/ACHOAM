...
export class SqliteLockManager {
    ...
    instanceAddress;
    dbPath;
    STALE_LOCK_TIMEOUT = 1 * 60 * 1000; // 1 minute in milliseconds
    constructor(options) {
    ...
    initializeDatabaseWithLockSync() {
    ...
    getInstanceByPort(port) {
		...
		`);
        const result = query.get(port, port);
        if (result) {
            return {
     ...
     */
    removeInstanceByAddress(instanceAddress) {
        const deleteLock = this.db.prepare(`
			DELETE FROM locks 
			WHERE held_by = ? AND lock_type = 'instance'
		`);
     ...
     */
    async getFolderLockByTarget(lockTarget) {
        const query = this.db.prepare(`
			SELECT * FROM locks 
			WHERE lock_type = 'folder' 
			AND lock_target = ? 
		`);
        const result = query.get(lockTarget);
        return result || null;
    }
    /**
     * Release a folder lock
     */
    releaseFolderLockByTarget(heldBy, lockTarget) {
        const deleteLock = this.db.prepare(`
			DELETE FROM locks 
     ...
     */
    async registerFolderLock(heldBy, lockTarget) {
        const now = Date.now();
        const insertLock = this.db.prepare(`
...
//# sourceMappingURL=SqliteLockManager.js.map