...
export class ClineHandler {
    ...
    async *createMessage(systemPrompt, messages, tools) {
        try {
            ...
            const stream = await createOpenRouterStream(client, systemPrompt, messages, this.getModel(), this.options.reasoningEffort, this.options.thinkingBudgetTokens, this.options.openRouterProviderSorting, tools, this.options.geminiThinkingLevel);
