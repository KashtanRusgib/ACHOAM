...
import { BedrockRuntimeClient, ConversationRole, ConverseCommand, ConverseStreamCommand, InvokeModelWithResponseStreamCommand, } from "@aws-sdk/client-bedrock-runtime";
...
export class AwsBedrockHandler {
    ...
    *handleBedrockStreamError(chunk) {
    ...
    shouldEnableReasoning(baseModelId, budgetTokens) {
        return ((baseModelId.includes("3-7") ||
     ...
     * Uses non-streaming Converse API and simulates streaming for models that don't support it
     */
    async *createQwenMessage(systemPrompt, messages, modelId, model) {
        // Get Bedrock client with proper credentials
        const client = await this.getBedrockClient();
        // Format messages for Converse API
        const formattedMessages = this.formatMessagesForConverseAPI(messages);
