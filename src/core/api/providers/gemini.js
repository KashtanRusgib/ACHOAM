...
import { ApiError, FunctionCallingConfigMode, GoogleGenAI, ThinkingLevel, } from "@google/genai";
                    ...
                    if (error.status === 429) {
                        ...
                        if (response && response.error) {
                            ...
                            if (responseBody.error) {
                                const detail = responseBody.error.details?.find((d) => d["@type"] === "type.googleapis.com/google.rpc.RetryInfo");
                                const detailedError = new RetriableError(apiError, this.parseRetryDelay(detail?.retryDelay) || undefined, {
                                    cause: error,
                                });
                                throw detailedError;
                            }
