...
import { HookInput, HookOutput, } from "../../shared/proto/cline/hooks";
 ...
 */
function validateHookOutput(output) {
    // Check if deprecated shouldContinue field is present
    if (output.shouldContinue !== undefined) {
        return {
            valid: false,
            error: "Invalid hook output: The 'shouldContinue' field has been removed.\n\n" +
    ...
    if (output.cancel !== undefined && typeof output.cancel !== "boolean") {
        return {
            valid: false,
            error: "Invalid hook output: 'cancel' must be a boolean.\n\n" +
    ...
    if (output.contextModification !== undefined && typeof output.contextModification !== "string") {
        return {
            valid: false,
            error: "Invalid hook output: 'contextModification' must be a string.\n\n" +
    ...
    if (output.errorMessage !== undefined && typeof output.errorMessage !== "string") {
        return {
            valid: false,
            error: "Invalid hook output: 'errorMessage' must be a string.\n\n" +
...
export class HookRunner {
    ...
    async completeParams(params) {
        const workspaceRoots = StateManager.get()
 ...
 */
class NoOpRunner extends HookRunner {
    /**
 ...
 * @template Name The type of hook this runner represents
 */
class StdioHookRunner extends HookRunner {
    scriptPath;
    streamCallback;
    abortSignal;
    constructor(hookName, scriptPath, streamCallback, abortSignal) {
        super(hookName);
    ...
    async [exec](input) {
        ...
        const jsonObj = HookInput.toJSON(input);
        ...
        if (this.streamCallback) {
            ...
            hookProcess.on("line", (line, stream) => {
            ...
            const parseJsonOutput = () => {
                try {
                    ...
                    if (output.contextModification && output.contextModification.length > MAX_CONTEXT_MODIFICATION_SIZE) {
                        console.warn(`Hook ${this.hookName} returned contextModification of ${output.contextModification.length} bytes, ` +
 ...
 * @template Name The type of hook this runner represents
 */
class CombinedHookRunner extends HookRunner {
    runners;
    constructor(hookName, runners) {
        super(hookName);
...
export class HookFactory {
     ...
     */
    async createWithStreaming(hookName, streamCallback, abortSignal) {
     ...
     */
    static async findHookInHooksDir(hookName, hooksDir) {
     ...
     * @throws Error if an unexpected file system error occurs
     */
    static async findUnixHook(hookName, hooksDir) {
        const candidate = path.join(hooksDir, hookName);
        try {
     ...
     */
    static handleHookDiscoveryError(error, hookName, candidate) {
        if (!isExpectedHookError(error)) {
            throw new Error(`Unexpected error while searching for hook '${hookName}' at '${candidate}': ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
//# sourceMappingURL=hook-factory.js.map