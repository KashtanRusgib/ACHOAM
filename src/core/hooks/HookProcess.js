...
export class HookProcess extends EventEmitter {
    ...
    exitCode = null;
    isCompleted = false;
    timeoutHandle = null; // 30-second execution timeout
    ...
    isRegistered = false;
    constructor(scriptPath, timeoutMs = 30000, abortSignal) {
        super();
        ...
        try {
            return await new Promise((resolve, reject) => {
                ...
                this.timeoutHandle = setTimeout(() => {
                    if (this.childProcess && !this.isCompleted) {
                        this.childProcess.kill("SIGTERM");
                        reject(new Error(`Hook execution timed out after ${this.timeoutMs}ms. The hook script at '${this.scriptPath}' took too long to complete.`));
                    }
                }, this.timeoutMs);
                // Handle stdout
     ...
     */
    handleOutput(data, _didEmitEmptyLine, stream) {
...
//# sourceMappingURL=HookProcess.js.map