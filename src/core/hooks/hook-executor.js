...
export async function executeHook(options) {
    const { hookName, hookInput, isCancellable, say, setActiveHookExecution, clearActiveHookExecution, messageStateHandler, taskId, hooksEnabled, } = options;
    ...
    try {
        ...
        const streamCallback = async (line) => {
            await say("hook_output", line);
        };
        // Create and execute hook
        const hook = await hookFactory.createWithStreaming(hookName, streamCallback, isCancellable ? abortController.signal : undefined);
 ...
 */
async function updateHookMessage(messageStateHandler, hookMessageTs, metadata) {
 ...
 */
async function reorderHookAndToolMessages(messageStateHandler) {
    const clineMessages = messageStateHandler.getClineMessages();
