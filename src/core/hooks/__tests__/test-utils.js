 ...
 */
export async function createHooksDirectory(baseDir) {
 ...
 * })
 */
export async function createTestHook(baseDir, hookName, output, options = {}) {
 ...
 */
function generateHookScript(output, options) {
 ...
 */
async function writeShellHook(hooksDir, hookName, scriptContent) {
 ...
 * })
 */
export function assertHookOutput(actual, expected) {
    if (expected.cancel !== undefined) {
        if (actual.cancel !== expected.cancel) {
            throw new Error(`Hook output assertion failed for 'cancel':\n` +
    ...
    if (expected.contextModification !== undefined) {
        if (actual.contextModification !== expected.contextModification) {
            throw new Error(`Hook output assertion failed for 'contextModification':\n` +
    ...
    if (expected.errorMessage !== undefined) {
        if (actual.errorMessage !== expected.errorMessage) {
            throw new Error(`Hook output assertion failed for 'errorMessage':\n` +
...
export class MockHookRunner {
    ...
    async run(params) {
        ...
        if (!isSerializable(params)) {
            throw new Error(`MockHookRunner: Cannot clone non-serializable input. ` +
    ...
    assertCalled(times) {
        if (this.executionLog.length !== times) {
            throw new Error(`MockHookRunner call count assertion failed:\n` +
    ...
    assertCalledWith(matcher) {
        ...
        if (matchingCalls.length === 0) {
            throw new Error(`MockHookRunner input assertion failed - no calls matched the expected input:\n` +
 ...
 */
export async function loadFixture(fixtureName, destDir) {
