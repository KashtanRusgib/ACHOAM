import { combineRuleToggles, getRuleFilesTotalContent, readDirectoryRecursive, synchronizeRuleToggles, } from "@core/context/instructions/user-instructions/rule-helpers";
 ...
 */
async function findAgentsMdFiles(cwd) {
    try {
 ...
 */
export const getLocalAgentsRules = async (cwd, toggles) => {
    ...
    try {
        ...
        const combinedContent = await Promise.all(agentsMdFiles.map(async (filePath) => {
            try {
