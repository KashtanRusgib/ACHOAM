...
export class ContextManager {
     ...
     */
    async getNewContextMessagesAndMetadata(apiConversationHistory, clineMessages, api, conversationHistoryDeletedRange, previousApiReqIndex, taskDirectory, useAutoCondense) {
            ...
            if (previousApiReqIndex >= 0) {
                ...
                if (previousRequest && previousRequest.text) {
                    ...
                    if (totalTokens >= maxAllowedSize) {
                        ...
                        let { anyContextUpdates, needToTruncate } = this.attemptFileReadOptimizationCore(apiConversationHistory, conversationHistoryDeletedRange, timestamp);
                        if (needToTruncate) {
     ...
     */
    ensureToolResultsFollowToolUse(messages) {
        ...
        if (startFromIndex > 2 && messagesToUpdate.length > 2) {
            ...
            if (firstMessageAfterTruncation.role === "user" && Array.isArray(firstMessageAfterTruncation.content)) {
                ...
                if (hasToolResults) {
                    ...
                    messagesToUpdate[2].content = firstMessageAfterTruncation.content.filter((block) => block.type !== "tool_result");
                }
            }
        }
     ...
     */
    attemptFileReadOptimizationCore(apiConversationHistory, conversationHistoryDeletedRange, timestamp) {
        const startIndex = conversationHistoryDeletedRange ? conversationHistoryDeletedRange[1] + 1 : 2;
        const [anyContextUpdates, uniqueFileReadIndices] = this.applyContextOptimizations(apiConversationHistory, startIndex, timestamp);
        if (!anyContextUpdates) {
            return { anyContextUpdates: false, needToTruncate: true };
        }
        const percentSaved = this.calculateContextOptimizationMetrics(apiConversationHistory, conversationHistoryDeletedRange, uniqueFileReadIndices);
        return {
     ...
     */
    async attemptFileReadOptimization(apiConversationHistory, conversationHistoryDeletedRange, clineMessages, previousApiReqIndex, taskDirectory) {
        ...
        const timestamp = previousRequest.ts;
        const { anyContextUpdates, needToTruncate } = this.attemptFileReadOptimizationCore(apiConversationHistory, conversationHistoryDeletedRange, timestamp);
        if (anyContextUpdates) {
            await this.saveContextHistory(taskDirectory);
        }
                ...
                if (firstBlockText) {
                    ...
                    if (result) {
                        ...
                        if (toolName === "read_file") {
                            ...
                            if (blockText) {
                                this.handlePotentialFileChangeToolCalls(i, filePath, blockText, fileReadIndices, contentBlockIndex);
                                foundNormalFileRead = true;
                            }
                        }
                    }
                    ...
                    if (!foundNormalFileRead) {
                        ...
                        for (const candidateIndex of [0, 1, 2]) {
                            ...
                            if (blockText) {
                                const [hasFileRead, filePaths] = this.handlePotentialFileMentionCalls(i, blockText, fileReadIndices, thisExistingFileReads, // file reads we've already replaced in this text in the latest version of this updated text
                                candidateIndex);
                                if (hasFileRead) {
     ...
     */
    handlePotentialFileMentionCalls(i, blockText, fileReadIndices, thisExistingFileReads, innerIndex) {
     ...
     */
    handleReadFileToolCall(i, filePath, fileReadIndices, contentBlockIndex, headerText) {
        const indices = fileReadIndices.get(filePath) || [];
     ...
     */
    handlePotentialFileChangeToolCalls(i, filePath, blockText, fileReadIndices, contentBlockIndex) {
    ...
    applyFileReadContextHistoryUpdates(fileReadIndices, messageFilePaths, apiMessages, timestamp) {
        ...
        const fileMentionUpdates = new Map(); // [baseText, prevFilesReplaced, innerIndex]
