...
function checkIsBedrockContextWindowError(error) {
    try {
        ...
        const message = String(error?.message || error?.error?.message || nestedMessage || "");
        // Check for ValidationException with HTTP 400
        const isValidationException = errorType === "ValidationException" ||
...
export function checkIsVercelContextWindowError(error) {
    try {
        ...
        const has400InMessage = errorMessage &&
