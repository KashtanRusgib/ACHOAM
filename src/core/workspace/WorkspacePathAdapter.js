...
export class WorkspacePathAdapter {
     ...
     */
    resolvePath(relativePath, workspaceHint) {
        ...
        // Multi-root mode
        const manager = this.config.workspaceManager;
     ...
     */
    getAllPossiblePaths(relativePath) {
        // Single-root mode
        if (!this.config.isMultiRootEnabled || !this.config.workspaceManager) {
     ...
     */
    getWorkspaceForPath(absolutePath) {
        // Single-root mode
        if (!this.config.isMultiRootEnabled || !this.config.workspaceManager) {
 ...
 */
export function createWorkspacePathAdapter(config) {
    return new WorkspacePathAdapter(config);
}
//# sourceMappingURL=WorkspacePathAdapter.js.map