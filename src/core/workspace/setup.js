 ...
 */
export async function setupWorkspaceManager({ stateManager, detectRoots, }) {
    ...
    try {
        ...
        if (multiRootEnabled) {
            ...
            // Telemetry
            telemetryService.captureWorkspaceInitialized(roots.length, roots.map((r) => r.vcs.toString()), performance.now() - startTime, true);
            // Persist
        ...
        // }
        manager = await WorkspaceRootManager.fromLegacyCwd(cwd);
        telemetryService.captureWorkspaceInitialized(1, [manager.getRoots()[0].vcs.toString()], performance.now() - startTime, false);
        ...
        telemetryService.captureWorkspaceInitError(error, true, workspaceCount);
