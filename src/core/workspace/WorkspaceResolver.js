...
export class WorkspaceResolver {
    usageMap = new Map();
    traceEnabled = process.env.MULTI_ROOT_TRACE === "true" || process.env.NODE_ENV === "development";
    /**
     ...
     */
    trackUsage(context, examplePath) {
     ...
     */
    resolveWorkspacePath(cwdOrRoots, relativePath, context) {
     ...
     */
    resolveSingleRootPath(cwd, relativePath, context) {
     ...
     */
    resolveMultiRootPath(workspaceRoots, relativePath) {
     ...
     * @returns Object with absolute path and matching root
     */
    resolveAbsolutePath(workspaceRoots, absolutePath) {
     ...
     * @returns Object with absolute path and matching root
     */
    resolveRelativePath(workspaceRoots, relativePath) {
     ...
     */
    selectBestRoot(workspaceRoots, candidateRoots, relativePath) {
 ...
 */
export function resolveWorkspacePath(cwdOrConfig, relativePath, context) {
    // Backward compatibility: if first param is a string, return string
    if (typeof cwdOrConfig === "string") {
        return workspaceResolver.resolveWorkspacePath(cwdOrConfig, relativePath, context);
    }
    ...
    const absolutePath = workspaceResolver.resolveWorkspacePath(config.cwd, relativePath, context);
    return {
        absolutePath,
 ...
 */
export function isWorkspaceTraceEnabled() {
 ...
 */
export function getWorkspaceBasename(filePath, context) {
