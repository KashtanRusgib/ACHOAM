...
describe("WorkspaceResolver", () => {
    ...
    describe("resolveWorkspacePath - Multi Root Mode", () => {
        ...
        it("should handle absolute paths in multi-root mode", () => {
            ...
            expect(result.absolutePath).to.equal(absolutePath);
            expect(result.root).to.equal(workspaceRoots[0]);
        });
        ...
        it("should resolve relative paths against primary root", () => {
            ...
            expect(result).to.be.an("object");
            expect(result.absolutePath).to.equal(path.resolve(workspaceRoots[0].path, relativePath));
            expect(result.root).to.equal(workspaceRoots[0]);
        });
...
//# sourceMappingURL=WorkspaceResolver.test.js.map