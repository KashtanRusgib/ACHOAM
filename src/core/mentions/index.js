...
export async function parseMentions(text, cwd, urlContentFetcher, fileContextTracker, workspaceManager) {
    ...
    for (const mention of uniqueMentions) {
        ...
        if (mention.startsWith("http")) {
            ...
            if (isMultiRoot && !workspaceHint) {
                ...
                const searchPromises = workspaceRoots.map(async (root) => {
                    try {
 ...
 */
function parseWorkspaceMention(mention) {
...
function getWorkspaceHintFromMention(mention) {
    const workspaceMention = parseWorkspaceMention(mention);
