...
export async function migrateTaskHistoryToFile(context) {
    try {
        // Get data from old location
        const vscodeGlobalStateTaskHistory = context.globalState.get("taskHistory");
        ...
        if (successfullyWrittenData.length !== finalData.length) {
            console.error("[Storage Migration] Failed to write taskHistory to file: Written data does not match the old location data");
            return;
        }
...
export async function cleanupMcpMarketplaceCatalogFromGlobalState(context) {
    try {
